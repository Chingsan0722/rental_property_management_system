...
export default function PropertyManagementForm({ caseId, onClose }: PropertyManagementFormProps) {
  ...
  }, [formData.monthly_rent, formData.management_fee_ratio]);
  ...
  const calculateManagementFee = () => {
    ...
    const ratio = Number(formData.management_fee_ratio);
    const fee = rent * (ratio / 100);
    setFormData(prev => ({ ...prev, management_fee: fee.toString() }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const dataToSave = {
        ...
        management_fee: Number(formData.management_fee),
    ...
    <div className="bg-white rounded-lg shadow-lg max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-lg font-semibold text-gray-700 mb-2">房客名稱</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-lg font-semibold text-gray-700 mb-2">房客電話</label>
            <input
              type="tel"
          ...
          <div>
            <label className="block text-lg font-semibold text-gray-700 mb-2">管理費</label>
            <div className="w-full px-4 py-3 text-lg bg-gray-100 border border-gray-300 rounded-lg font-semibold text-blue-600">
              NT$ {Number(formData.management_fee).toLocaleString()}
            </div>
          </div>
